    ...
    """
     ...
     if symbol:
        ...
        if stock_data is not None:
            ...
            price_difference, percentage_difference, latest_close_price, max_52_week_high, min_52_week_low = None, None, None, None, None

        col1, col2, col3, col4 = st.columns(4)
        with col1:
            st.metric("Close Price", f"${latest_close_price:.2f}" if latest_close_price is not None else "N/A")
        with col2:
            st.metric("Price Difference (YoY)", f"${price_difference:.2f}" if price_difference is not None else "N/A",
                    f"{percentage_difference:+.2f}%" if percentage_difference is not None else "N/A")
        with col3:
            st.metric("52-Week High", f"${max_52_week_high:.2f}" if max_52_week_high is not None else "N/A")
        with col4:
